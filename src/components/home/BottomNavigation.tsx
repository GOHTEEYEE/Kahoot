...
export function BottomNavigation() {
  ...
  return (
    <nav className="pointer-events-none fixed inset-x-0 bottom-0 z-40 px-3 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
      <ul className="nav-dock pointer-events-auto mx-auto flex h-[3.65rem] max-w-[500px] items-stretch justify-between gap-0.5 rounded-t-[1.25rem] rounded-b-[1.75rem] px-2 py-1">
        {TABS.map((tab) => {
          const active =
            tab.href === "/"
              ? pathname === "/"
            ...
            <li key={tab.href} className="flex flex-1 items-stretch">
              <Link
                ...
                className={`flex w-full flex-col items-center justify-center gap-0.5 rounded-[0.9rem] px-0.5 py-1 text-center transition-transform duration-200 ${
                  active ? "nav-tab-active text-[#2a2118]" : "text-[#8a7355]"
                }`}
              ...
              >
                <motion.span
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-xl ${
                    active ? "bg-[rgba(180,230,160,0.55)]" : "opacity-75 saturate-[0.88]"
                  }`}
                  aria-hidden
